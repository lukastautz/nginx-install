...
sudo apt-get -y install php8.0-fpm php8.0-common php8.0-gmp php8.0-curl php8.0-intl php8.0-mbstring php8.0-xmlrpc php8.0-gd php8.0-xml php8.0-zip php8.0-soap php8.0-imap
sudo rm -r /var/www/html
sudo rm -r /etc/apache2
sudo chmod 777 -R -v /var
sudo chmod 777 -R -v /etc/php/8.0/fpm
...
sudo echo "        fastcgi_pass unix:/var/run/php/php8.0-fpm.sock;" >> /etc/nginx/sites-available/default
sudo echo "        }" >> /etc/nginx/sites-available/default
sudo echo "}" >> /etc/nginx/sites-available/default
...
sudo echo 'error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT' >> /etc/php/8.0/fpm/php.ini
sudo echo 'display_errors = Off' >> /etc/php/8.0/fpm/php.ini
sudo echo 'display_startup_errors = Off' >> /etc/php/8.0/fpm/php.ini
sudo echo 'log_errors = On' >> /etc/php/8.0/fpm/php.ini
sudo echo 'log_errors_max_len = 1024' >> /etc/php/8.0/fpm/php.ini
...
sudo echo 'mysqlnd.collect_memory_statistics = Off' >> /etc/php/8.0/fpm/php.ini
...
sudo echo 'session.gc_probability = 0' >> /etc/php/8.0/fpm/php.ini
sudo echo 'session.gc_divisor = 1000' >> /etc/php/8.0/fpm/php.ini
sudo echo 'session.gc_maxlifetime = 1440' >> /etc/php/8.0/fpm/php.ini
...
sudo echo '[curl]' >> /etc/php/8.0/fpm/php.ini
sudo echo '[openssl]' >> /etc/php/8.0/fpm/php.ini
sudo echo '[ffi]' >> /etc/php/8.0/fpm/php.ini
